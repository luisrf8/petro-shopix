    ...
    <div class="card-body p-3">
      <form method="GET" action="{{ route('logs.index') }}" class="row g-2">
        <div class="col-12 col-md-2">
          <input type="text" class="form-control" name="user_id" placeholder="Usuario ID" value="{{ $filters['user_id'] ?? '' }}">
        </div>
        <div class="col-12 col-md-2">
          <input type="text" class="form-control" name="tenant_id" placeholder="Tienda ID" value="{{ $filters['tenant_id'] ?? '' }}">
        </div>
        <div class="col-12 col-md-2">
        ...
        <div class="col-12 col-md-6">
          <input type="text" class="form-control" name="q" placeholder="Buscar por texto libre (ruta, mensaje, payload, etc.)" value="{{ $filters['q'] ?? '' }}">
        </div>
          ...
          <tbody>
            @foreach ($logs as $log)
              ...
              <tr>
                ...
                <td>{{ $log->user_id }}</td>
                <td>{{ $payload['tenant_id'] ?? '-' }}</td>
                <td>{{ $payload['role'] ?? '-' }}</td>
                <td>{{ $payload['module'] ?? $log->table_name }}</td>
                <td>{{ $log->action }}</td>
                <td>{{ $payload['path'] ?? '-' }}</td>
                <td>{{ $payload['status'] ?? '-' }}</td>
                <td class="text-start" style="max-width: 560px; white-space: normal; word-break: break-word;">
                  @if($payload)
