...
class SaleController extends Controller
{
    ...
    public function sendDeliveryPdfToCustomerWhatsapp(SalesOrder $order, Request $request)
    {
        $user = auth()->user();
        if (!$user?->hasStoreRole('owner', 'admin', 'seller')) {
            return response()->json(['success' => false, 'message' => 'No autorizado para enviar por WhatsApp.'], 403);
        }
        ...
        $whatsappEnabled = (bool) config('services.whatsapp.enabled', false);
        ...
        $defaultCountryCode = preg_replace('/\D+/', '', (string) config('services.whatsapp.default_country_code', ''));
        $graphBaseUrl = rtrim((string) config('services.whatsapp.base_url', 'https://graph.facebook.com'), '/');
        $graphVersion = trim((string) config('services.whatsapp.version', 'v20.0'));

        if (!$whatsappEnabled || $phoneNumberId === '' || $accessToken === '') {
            return response()->json([
                'success' => false,
                'message' => 'La integración de WhatsApp no está configurada en el servidor.',
            ], 422);
        }

        $customerPhone = preg_replace('/\D+/', '', (string) ($order->user->phone_number ?? ''));
        if (is_string($customerPhone) && str_starts_with($customerPhone, '00')) {
            $customerPhone = substr($customerPhone, 2);
        }
        if (is_string($customerPhone) && str_starts_with($customerPhone, '0') && $defaultCountryCode !== '') {
            $customerPhone = $defaultCountryCode . ltrim($customerPhone, '0');
        }
        if (!is_string($customerPhone) || strlen($customerPhone) < 8 || strlen($customerPhone) > 15) {
            return response()->json([
                'success' => false,
                'message' => 'El cliente no tiene un número de WhatsApp válido para el envío.',
            ], 422);
        }

        try {
            $assets = $this->ensureAssociatedPdfAssets($order, true, 'delivery');
            $deliveryPath = (string) ($assets['delivery_path'] ?? '');
            if ($deliveryPath === '' || !is_file($deliveryPath)) {
                return response()->json([
                    'success' => false,
                    'message' => 'No se pudo generar el PDF de entrega para enviar por WhatsApp.',
                ], 422);
            }

            $pdfName = 'orden-entrega-' . (int) $order->id . '.pdf';
            $binary = @file_get_contents($deliveryPath);
            if ($binary === false || $binary === '') {
                return response()->json([
                    'success' => false,
                    'message' => 'No se pudo leer el PDF de entrega para enviarlo por WhatsApp.',
                ], 422);
            }

            $uploadResponse = Http::withToken($accessToken)
                ->timeout(45)
                ->attach('file', $binary, $pdfName)
                ->asMultipart()
                ->post($graphBaseUrl . '/' . $graphVersion . '/' . $phoneNumberId . '/media', [
                    ['name' => 'messaging_product', 'contents' => 'whatsapp'],
                    ['name' => 'type', 'contents' => 'application/pdf'],
                ]);

            if (!$uploadResponse->successful()) {
                Log::warning('Fallo subida de PDF a WhatsApp media API.', [
                    'order_id' => (int) $order->id,
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'WhatsApp rechazó la carga del documento PDF.',
                ], 422);
            }

            $mediaId = trim((string) ($uploadResponse->json('id') ?? ''));
            if ($mediaId === '') {
                return response()->json([
                    'success' => false,
                    'message' => 'WhatsApp no devolvió el identificador del documento.',
                ], 422);
            }

            $caption = 'Orden de entrega #' . (int) $order->id . ' - ' . ($order->tenant->name ?? 'Shopix');

            $messageResponse = Http::withToken($accessToken)
                ->timeout(45)
                ->post($graphBaseUrl . '/' . $graphVersion . '/' . $phoneNumberId . '/messages', [
            ...
            if (!$messageResponse->successful()) {
                Log::warning('Fallo envío de documento por WhatsApp.', [
                    'order_id' => (int) $order->id,
                    'tenant_id' => (int) ($order->tenant_id ?? 0),
                    'status' => (int) $messageResponse->status(),
                    'body' => $messageResponse->body(),
                ]);

                return response()->json([
                    'success' => false,
                    'message' => 'WhatsApp no pudo enviar el documento al cliente.',
                ], 422);
            }

            return response()->json([
                'success' => true,
                'message' => 'Orden de entrega enviada por WhatsApp al cliente.',
            ]);
        } catch (\Throwable $exception) {
            Log::error('Error al enviar PDF de entrega por WhatsApp.', [
                'order_id' => (int) $order->id,
                'tenant_id' => (int) ($order->tenant_id ?? 0),
                'error' => $exception->getMessage(),
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'No se pudo enviar la orden por WhatsApp en este momento.',
            ], 500);
        }
    }
